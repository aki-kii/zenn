...
CDK のスナップショットテストでは基本的には`cdk.json`の機能フラグを利用せずに実施しています。\
厳密には CDK を初期化した時と同じ機能フラグの設定が適用されます。\
これはプロジェクトで利用している機能フラグと異なる場合があります。すると破壊的変更がスナップショットにも反映されているかもしれませんね。これはまずそうです。
